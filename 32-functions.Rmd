...
Supposons maintenant que l'on veuille transformer notre base de donnée pour la faire passer du niveau des IRIS (i) à celui des communes (i2). Cela va entraîner une modification des fichiers tab et geo mais pas du fichier soc qui demeurera identique. Pour des raisons de compatibilité il faudra impérativement fournir un label à la nouvelle variable géographique, quitte à dupliquer le code. 
...
base_agreg_geo <- function(base = base,
 ...
 b$geo <- b$geo %>% mutate(i = {{var}}, lab_i = {{lab}}) %>%
                    group_by(i) %>%
